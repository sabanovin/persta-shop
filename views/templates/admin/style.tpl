<style>

@font-face {
  font-family: Vazir;
  src: url('../modules/sabanovin/font/Vazir.eot');
  src: url('../modules/sabanovin/font/Vazir.eot?#iefix') format('embedded-opentype'),
       url('../modules/sabanovin/font/Vazir.woff') format('woff'),
       url('../modules/sabanovin/font/Vazir.ttf') format('truetype');
  font-weight: normal;
}

@font-face {
  font-family: Vazir;
  src: url('../modules/sabanovin/font/Vazir-Bold.eot');
  src: url('../modules/sabanovin/font/Vazir-Bold.eot?#iefix') format('embedded-opentype'),
       url('../modules/sabanovin/font/Vazir-Bold.woff') format('woff'),
       url('../modules/sabanovin/font/Vazir-Bold.ttf') format('truetype');
  font-weight: bold;
}

.sabanovin *{
font-family:vazir!important;
}

#A_1 {
    box-shadow: rgb(161, 223, 250) 0px 1px 0px 0px inset;
    color: rgb(255, 255, 255);
    cursor: pointer;
    display: inline-block;
    height: 30px;
    letter-spacing: 0.300000011920929px;
    text-align: center;
    text-decoration: none;
    text-shadow: rgb(119, 119, 119) 1px 1px 0px;
    text-transform: uppercase;
    /*width: 143.46875px;*/
    perspective-origin: 92.734375px 21px;
    transform-origin: 92.734375px 21px;
    background: rgba(0, 0, 0, 0) -webkit-gradient(linear, 0% 0%, 0% 100%, color-stop(0.05, rgb(103, 182, 244)), to(rgb(58, 143, 206))) repeat scroll 0% 0% / auto padding-box border-box;
    border: 1px solid rgb(80, 143, 205);
    border-radius: 3px 3px 3px 3px;
    font: normal normal bold normal 15px/15px arial;
    margin: 21px 0px 0px;
    outline: rgb(255, 255, 255) none 0px;
    padding: 5px 20px;
    transition: all 0.1s ease 0s;
}



#B_1 {
    box-shadow: rgb(251, 175, 93) 0px 1px 0px 0px inset;
    color: rgb(255, 255, 255);
    cursor: pointer;
    display: inline-block;
    width : 250px;
    text-align: center;
    text-decoration: none;
    text-shadow: rgb(119, 119, 119) 1px 1px 0px;
    background: rgba(0, 0, 0, 0) -webkit-gradient(linear, 0% 0%, 0% 100%, color-stop(0.05, rgb(255, 143, 0)), to(rgb(255, 107, 0))) repeat scroll 0% 0% / auto padding-box border-box;
    border: 1px solid rgb(242, 101, 34);
    border-radius: 3px 3px 3px 3px;
    font: normal normal bold normal 15px/normal arial;
    outline: rgb(255, 255, 255) none 0px;
    padding: 5px 20px;
}


.video-container {
	position:relative;
	padding-bottom:56.25%;
	padding-top:30px;
	height:0;
	overflow:hidden;
}

.video-container iframe, .video-container object, .video-container embed {
	position:absolute;
	top:0;
	left:0;
	width:90%;
	height:90%;
}


.mb30 {
	margin-bottom:30px;
}

.mt30 {
	margin-top:30px;
}

.mt50 {
	margin-top:50px;
}

</style>
